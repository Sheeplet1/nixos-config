...
function __bobthefish_prompt_git -S -a git_root_dir -a real_pwd -d 'Display the Git or colocated Jujutsu state'
    if test -d "$git_root_dir/.jj"
        # Jujutsu dirtiness combines snapshotted @ changes with Git-visible staged,
        # unstaged, and untracked work so neither storage layer can appear falsely clean.
        set -l jj_dirty (command jj log --ignore-working-copy --no-pager --color never \
            -R "$git_root_dir" -r '@ & ~empty()' --no-graph -T '"dirty"' 2>/dev/null)
        if not [ "$jj_dirty" ]
            set jj_dirty (command git -C "$git_root_dir" status --porcelain 2>/dev/null)
        end
