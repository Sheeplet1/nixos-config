...
analog_sink=$(pactl list short sinks | grep 'alsa_output.pci-0000_08_00.4.analog-stereo' | awk '{print $2}')
...
if [ "$current_sink" == "$g560_sink" ]; then
    echo "Switching to headphones $analog_sink"
    ...
    echo "Switching to g560 $g560_sink"
